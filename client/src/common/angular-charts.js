...
angular.module('angularCharts').directive('acChart', function($templateCache, $compile, $rootElement, $window, $timeout, $sce) {
  ...
  function link(scope, element, attrs) {
    ...
    function lineChart() {
       ...
       * @type {[type]}
       */
      if (config.isAnimate && linedata.length > 0) {
        for (var i = 0, len = path[0].length; i < len; i++) {
          var totalLength = path[0][i].getTotalLength();
          path[0][i].style.transition = path[0][i].style.WebkitTransition = 'none';
          path[0][i].style.strokeDasharray = totalLength + ' ' + totalLength;
          path[0][i].style.strokeDashoffset = totalLength;
          path[0][i].getBoundingClientRect();
          path[0][i].style.transition = path[0][i].style.WebkitTransition = 'stroke-dashoffset 1.5s ease-in-out';
          path[0][i].style.strokeDashoffset = '0';
        }
      }
    ...
    $timeout(function() {
      ...
      w.bind('resize', function(ev) {
        if (elementWidth != element[0].clientWidth || elementHeight != element[0].clientHeight) {
    ...
    scope.$watch('acRefresh', init, true);
  }
